...
5. **Expand every collapsed accordion BEFORE capturing.** On both PSI and GTmetrix the findings are hidden inside collapsed drop-downs — a screenshot of the collapsed report is nearly useless for analysis, because the per-file sizes and flagged URLs are exactly what's inside them.
6. **Capture every relevant GTmetrix tab, not just the landing one** — at minimum **Summary, Performance, Structure, CrUX, and Waterfall**. Each is a separate screenshot (or several).
7. **Expect multiple screenshots per tool — that's normal.** Send them all to Claude Code rather than summarising or re-typing; the raw capture carries the waterfall, per-file sizes and flagged items the analysis depends on. If one is too tall to attach, split it into 2–3 vertical slices.
