...
- **Idempotency key on `/cal/book`** so a retry after a *false-negative* (request created the booking but the response was lost) can't create a duplicate. Lower urgency — the observed failures were clean (no record), but it's the right long-term hardening. **SHELVED 2026-08-07 (Victor): bundle it with the Phase 6 Cloudflare KV setup** — it needs a KV store (key→uid, ~10-min TTL) anyway, so do it when KV/D1 get bound for BI/reporting rather than standing up KV mid-launch just for this. Design: client generates a UUID once per booking attempt and **reuses it on retry**; `/cal/book` does `KV.get(key)` → return the existing booking if found, else create + `KV.put(key, uid, {expirationTtl: 600})`.
- **Layer 3 for client network-drops → Slack** — **DECIDED AGAINST 2026-08-07 (Victor):** network-drop failures stay in **GA4 only** (`booking_failed`, `error_type: network`). On flaky mobile they'd be frequent enough to make the Slack channel noise; Slack is reserved for *actionable* server-seen failures (`cal_error`/`timeout`). Revisit only if the GA4 network-drop rate is high enough to warrant real-time pings.
