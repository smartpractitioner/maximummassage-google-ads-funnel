...
| **5** | **Rollout: lymphatic, deep tissue, therapeutic** | 🔄 **lymphatic ~content/design-complete** (2026-08-04: desktop layer + guarantee band, per-breakpoint real hero + CTA imagery, 3 re-sourced reviews + mobile carousel, benefits icons + white-card desktop matching prenatal, FAQ incl. "are men…", session-steps band, quiz weights + therapist profiles verified) — **remaining: E2E + two-pass QA**. **Deep-tissue: ✅ COMPLETE + E2E PASSED (2026-08-06)** — full build (ch1–4: desktop layer, guarantee band, healing-inner, carousel, benefit icons, best-fit reviews, mobile hero flip) + quiz restructure (Decisions 11/16; Q1 pressure primer → duration → location multi → preference single "what matters most") + voice sweep (lean-in closes fixed). E2E: Victor verified Charlotte+Brookelyn booking UX both devices + events/attribution (`booking_confirmed`×1, UTMs→`leads_deep_tissue`+Jane+GA4); `bookingMode:calcom` live, Tif on notify-me (no Cal.com yet). **Therapeutic: 🔄 built (2026-08-06)** — core-anchor page at `/therapeutic-massage-calgary/`, all 5 therapists, full copy + quiz + bios; placeholder hero (`woman-massage.webp`); `calcom` live (E2E-ready 2026-08-07; Charlotte/Brookelyn/Meagan/Lindsey live, Tif notify-me); splitter cutover of `/massage-therapy-calgary/` is a separate later step. |
...
| Therapeutic — new build + splitter cutover + E2E | 🔄 **built (2026-08-06)** at `/therapeutic-massage-calgary/` — core-anchor page, all 5 therapists, quiz (5 Qs, Decisions 11/16: intent→impact→focus[multi]→tried[multi]→preference[single]), 5 `skills.therapeutic` bios (calm-confidence voice), full copy (hero/intro/6 benefits/session-steps/10 FAQ/CTA), cloned from deep-tissue template (desktop layer + carousel + guarantee band). Placeholder hero/CTA = `woman-massage.webp` (dedicated image = follow-up). `bookingMode:calcom` live (E2E-ready 2026-08-07). **Remaining: E2E + the SEPARATE splitter cutover** (back up splitter + Flow B, then point `/massage-therapy-calgary/` here). Also fixed a shared-guarantee-band em-dash regression here (still present on deep-tissue + lymphatic). |
