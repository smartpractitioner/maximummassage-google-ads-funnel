...
| Deep tissue — full treatment + E2E | 🔄 **chunks 1–3 done.** Ch1 (2026-08-04): CTA unify, drop $124, scarcity; real hero + CTA images; FAQ de-named. Ch2 (2026-08-05): desktop layer (≥1024) + guarantee band + `.healing-inner` + mobile review carousel (CSS/dots/JS) ported from lymphatic; tablet session-steps band fixed; hero `object-position` tuned per-page for the portrait hero (**QA-nudge on desktop**); reviews are lymphatic placeholders (swap in ch4); + highlighter on the modality-intro "targeted change, not relaxation" line. Ch3 (2026-08-05): benefits now have 6 themed pink line-icons + `.benefit-text` flex structure → white cards on desktop with the number inline-left (fixes the ch2 number-stacking caveat). Ch4 (2026-08-05): swapped the 2 lymphatic placeholders for best-fit deep-tissue reviews from the client-assets pool (Chayada — "Therapeutic Deep Tissue" peer endorsement; Janet — "loose as a goose" mobility payoff); Marc kept as card 1. Ch5-quiz (2026-08-05): applied Decision 11 single/multi classification (the deferred Phase-5 call) **and** wrote new **Decision 16** (Q1 is always a single-select auto-advance primer for completion bias; multi questions go later). Deep-tissue quiz restructured (kept at **4 Qs**): Q1 = the existing `pressure` question promoted (genuinely single per D11, engaging, and doesn't echo the symptom Q — the interim `intent` draft was dropped as a reworded duplicate of `location`), `location` → Q3 multi (off "most"), `preference` → multi + `exclusive` none; `duration` single. Tif flip requested but **E2E-blocked: `cal.com/thenderson` 404s (she has no Cal.com account)** — reverted to `active: false` (demand-test "notify me"); needs her real handle before she can go live. Charlotte + Brookelyn Cal handles resolve (200). **Deep-tissue flipped `bookingMode: 'demand_test' → 'calcom'`** (Phase 5 rollout, per Victor) — Charlotte/Brookelyn book live, Tif notify-me. **Chunk 5 = E2E in progress**: automatable slice done (page 200/live, new 4-Q quiz + calcom flip served, Cal handles checked); argmax per-therapist combos computed (charlotte/brookelyn/tif); **remaining = the human walk** (real phone → booking → Jane → GTM/GA4 attribution, per `docs/sop-e2e-launch-testing.md`). **Engine extraction (desktop layer + carousel → `flow-b-v3.css`) is deferred to a dedicated refactor** so it can't break prenatal/lymphatic. |
